...
fun RoomCreatorPopup(context: Context, serverDomain: String, userName: String, accessToken: String, refreshToken: String?, onClose: () -> Unit, onRoomCreated: (roomId: String) -> Unit) {
    ...
    fun create() {
        if (inviteUserName == "" && (!directMessageRoom && roomName == "")) {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            val result = if(directMessageRoom) matrixClient.createPrivateChat(inviteUserName) else matrixClient.createRoom(roomName, RoomVisibility.PRIVATE, inviteUserName)
            CoroutineScope(Dispatchers.Default).launch {
                if(!result.success) {
                    ...
                    else createStatus = CreateStatus.UNAUTHORIZED
        ...
        Surface(
            ...
            Column(
                ...
                EditStringField(text = "user to invite",
                    ...
                    onValueChanged = {inviteUserName = it},
                    modifier = Modifier.fillMaxWidth()
                )
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            CoroutineScope(Dispatchers.Main).launch {
                if(!result.success) {
                    ...
                    else selectStatus = SelectStatus.UNAUTHORIZED
                } else {
                    rooms = result.value!!
                }
            }
